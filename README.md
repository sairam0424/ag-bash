...
See [bash-tool](https://github.com/ag-ai/bash-tool) for more.
...
Install globally (`npm install -g @ag/bash`) for a sandboxed CLI:
...
- Use a full VM environment if you need arbitrary binary execution.
...
For AI agents, [`bash-tool`](https://github.com/ag-ai/bash-tool) provides additional guidance in its `AGENTS.md`:
