...
import { AgentAdapter, UIMessage, TerminalWriter } from './index.js';
...
describe('AgentOrchestrator', () => {
  ...
  it('should process bash commands from adapter', async () => {
     const mockAdapter: AgentAdapter = {
      ...
      async *run(messages) {
        ...
        yield { 
          ...
          toolCallId: 'call1', 
          output: JSON.stringify({ stdout: 'file1.txt', stderr: '', exitCode: 0 })
        };
        yield { type: 'text-end' };
      }
    };
    ...
    expect(mockWriter.write).toHaveBeenCalledWith(expect.stringContaining('file1.txt'));
  });
});
