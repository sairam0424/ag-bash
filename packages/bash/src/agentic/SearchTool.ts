...
export const GrepTool: ToolboxTool = buildTool({
  ...
  parameters: z.object({
    ...
    ignoreCase: z.boolean().optional().describe("Whether to ignore case when searching."),
  }),
  isReadOnly: true,
...
export const FindFilesTool: ToolboxTool = buildTool({
  ...
  parameters: z.object({
    ...
    path: z.string().optional().describe("The directory to start searching from (default: current directory)."),
  }),
  isReadOnly: true,
  execute: async (bash: Bash, args: any) => {
