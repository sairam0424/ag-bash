...
vi.mock("../../parser/tree-sitter-to-ast.js", () => {
  return {
    TreeSitterToAst: vi.fn().mockImplementation(function() {
      return {
        convert: () => ({
          type: "Script",
          statements: [
...
describe("ag-analyze", () => {
  ...
  it("should show basic file info for non-bash files", async () => {
    ...
    expect(result.stdout).toContain("Deep semantic analysis currently only supported for Bash scripts");
  });
  ...
  it("should output JSON symbols with --symbols", async () => {
    const result = await agAnalyzeCommand.execute(["script.sh", "--symbols"], ctx);
    try {
      ...
      throw new Error(`JSON parse failed! stdout: ${result.stdout} stderr: ${result.stderr} err: ${e}`);
    }
  });
});
