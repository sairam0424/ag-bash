...
describe("ag-edit", () => {
  ...
  it("should insert lines before a specific line", async () => {
    const result = await agEditCommand.execute(
      ["insert-before", "test.txt", "-n", "2", "-x", "new line"],
      ctx,
    );
  ...
  it("should replace a range of lines", async () => {
    const result = await agEditCommand.execute(
      ["replace", "test.txt", "-n", "1", "-t", "2", "-x", "replaced"],
      ctx,
    );
    expect(result.exitCode).toBe(0);
    const content = await fs.readFile("/test.txt", "utf8");
  ...
  it("should support dry-run mode", async () => {
    const result = await agEditCommand.execute(
      ["append", "test.txt", "-x", "appended", "--dry-run"],
      ctx,
    );
    expect(result.exitCode).toBe(0);
  ...
  it("should return error for invalid line numbers", async () => {
    const result = await agEditCommand.execute(
      ["insert-before", "test.txt", "-n", "10", "-x", "fail"],
      ctx,
    );
