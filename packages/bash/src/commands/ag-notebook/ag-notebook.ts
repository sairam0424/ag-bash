  ...
  execute: async (args: string[], ctx: CommandContext): Promise<ExecResult> => {
    ...
    try {
      ...
      if (!(await ctx.fs.exists(filePath))) {
        return { stdout: "", stderr: `File not found: ${path} (resolved to ${filePath})\n`, exitCode: 1 };
      }

      const content: NotebookContent = JSON.parse(await ctx.fs.readFile(filePath, "utf8"));
      ...
      switch (subcommand) {
        ...
        case "edit": {
          const cellIndex = parseInt(args[2]);
          const newSource = args.slice(3).join(" ");
          if (isNaN(cellIndex) || !newSource) {
            return { stdout: "", stderr: "Usage: ag-notebook edit <path> <index> <content>\n", exitCode: 1 };
          }
          ...
          return { stdout: `Successfully updated cell ${cellIndex}.\n`, stderr: "", exitCode: 0 };
        }
        ...
        case "add": {
          ...
          if (!type || !source) {
            return { stdout: "", stderr: "Usage: ag-notebook add <path> <code|markdown> <content>\n", exitCode: 1 };
          }
          ...
          await ctx.fs.writeFile(filePath, JSON.stringify(content, null, 2));
          return { stdout: `Successfully added new ${type} cell.\n`, stderr: "", exitCode: 0 };
        }
