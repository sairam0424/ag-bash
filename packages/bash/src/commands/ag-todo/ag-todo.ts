  ...
  async execute(args: string[], ctx: CommandContext): Promise<ExecResult> {
    ...
    switch (subcommand) {
      case "list": {
        ...
        todos.forEach((t) => {
          const marker =
            t.status === "done" ? "[x]" : t.status === "doing" ? "[/]" : "[ ]";
      ...
      case "add": {
        ...
        if (!task)
          return {
            stdout: "",
            stderr: "ag-todo: missing task description\n",
            exitCode: 1,
          };
        const todos = await loadTodos();
        const newTodo: Todo = {
      ...
      case "update": {
        ...
        if (!id || !["pending", "doing", "done"].includes(status)) {
          return {
            stdout: "",
            stderr: "Usage: ag-todo update <id> <pending|doing|done>\n",
            exitCode: 1,
          };
        }
        const todos = await loadTodos();
        ...
        if (!todo)
          return {
            stdout: "",
            stderr: `ag-todo: todo ${id} not found\n`,
            exitCode: 1,
          };
        todo.status = status;
        await saveTodos(todos);
        return {
          stdout: `Updated todo ${id} status to ${status}\n`,
          stderr: "",
          exitCode: 0,
        };
      }
      ...
      default:
        return {
          stdout: "",
          stderr: `ag-todo: unknown subcommand: ${subcommand}\n`,
          exitCode: 1,
        };
    }
  },
