...
export function formatPrintf(format, values) {
    let valueIdx = 0;
    let result = "";
    let i = 0;
        ...
        if (format[i] === "%" && i + 1 < format.length) {
            ...
            if (format[j] === ".") {
                ...
                if (format[j] === "*") {
                    ...
                    precision = String(Math.min(precVal !== undefined ? Math.floor(Number(precVal)) : 0, MAX_PRINTF_WIDTH));
                    j++;
                }
                else {
                    while (j < format.length && /\d/.test(format[j])) {
...
export const awkBuiltins = new Map([
    // String functions
    ["length", awkLength],
    ["substr", awkSubstr],
    ["index", awkIndex],
