...
export async function evalExpr(ctx, expr) {
    ...
    switch (expr.type) {
        ...
        case "ternary":
            return isTruthy(await withDefenseContext(ctx, "ternary condition evaluation", () => evalExpr(ctx, expr.condition)))
                ? await withDefenseContext(ctx, "ternary consequent evaluation", () => evalExpr(ctx, expr.consequent))
                : await withDefenseContext(ctx, "ternary alternate evaluation", () => evalExpr(ctx, expr.alternate));
        case "call":
...
async function evalFieldRef(ctx, expr) {
    ...
    const index = Math.floor(toNumber(await withDefenseContext(ctx, "field index evaluation", () => evalExpr(ctx, expr.index))));
    return getField(ctx, index);
}
async function evalArrayAccess(ctx, expr) {
    assertAwkDefenseContext(ctx, "array access evaluation");
    const key = toAwkString(await withDefenseContext(ctx, "array key evaluation", () => evalExpr(ctx, expr.key)));
...
async function evalBinaryOp(ctx, expr) {
    ...
    if (op === "||") {
        return isTruthy(await withDefenseContext(ctx, "logical-or left evaluation", () => evalExpr(ctx, expr.left))) ||
            isTruthy(await withDefenseContext(ctx, "logical-or right evaluation", () => evalExpr(ctx, expr.right)))
            ? 1
            : 0;
    }
    if (op === "&&") {
        return isTruthy(await withDefenseContext(ctx, "logical-and left evaluation", () => evalExpr(ctx, expr.left))) &&
            isTruthy(await withDefenseContext(ctx, "logical-and right evaluation", () => evalExpr(ctx, expr.right)))
            ? 1
            : 0;
    }
    // Regex match operators - handle regex literal specially
    if (op === "~") {
        const left = await withDefenseContext(ctx, "regex left evaluation", () => evalExpr(ctx, expr.left));
        if (expr.right.type === "regex")
            ctx.coverage?.hit("awk:expr:regex");
        const pattern = expr.right.type === "regex"
            ? expr.right.pattern
            : toAwkString(await withDefenseContext(ctx, "regex right evaluation", () => evalExpr(ctx, expr.right)));
        try {
    ...
    if (op === "!~") {
        const left = await withDefenseContext(ctx, "negated-regex left evaluation", () => evalExpr(ctx, expr.left));
        if (expr.right.type === "regex")
            ctx.coverage?.hit("awk:expr:regex");
        const pattern = expr.right.type === "regex"
            ? expr.right.pattern
            : toAwkString(await withDefenseContext(ctx, "negated-regex right evaluation", () => evalExpr(ctx, expr.right)));
        try {
...
async function evalAssignment(ctx, expr) {
    ...
    if (op === "=") {
        ...
        if (target.type === "field") {
            const index = Math.floor(toNumber(await withDefenseContext(ctx, "assignment field index", () => evalExpr(ctx, target.index))));
            ...
            const key = toAwkString(await withDefenseContext(ctx, "assignment array key", () => evalExpr(ctx, target.key)));
    ...
    // Assign to target
    if (target.type === "field") {
        const index = Math.floor(toNumber(await withDefenseContext(ctx, "assignment target field index", () => evalExpr(ctx, target.index))));
        ...
        const key = toAwkString(await withDefenseContext(ctx, "assignment target array key", () => evalExpr(ctx, target.key)));
...
async function applyIncDec(ctx, operand, delta, returnNew) {
    ...
    if (operand.type === "field") {
        const index = Math.floor(toNumber(await withDefenseContext(ctx, "inc/dec field index", () => evalExpr(ctx, operand.index))));
        ...
        const key = toAwkString(await withDefenseContext(ctx, "inc/dec array key", () => evalExpr(ctx, operand.key)));
...
async function evalInExpr(ctx, key, array) {
    ...
    if (key.type === "tuple") {
        ...
        for (const e of key.elements) {
            parts.push(toAwkString(await withDefenseContext(ctx, "tuple key element evaluation", () => evalExpr(ctx, e))));
        }
        keyStr = parts.join(ctx.SUBSEP);
    }
    else {
        keyStr = toAwkString(await withDefenseContext(ctx, "in-expression key evaluation", () => evalExpr(ctx, key)));
    }
...
async function evalGetlineFromCommand(ctx, variable, cmdExpr) {
    ...
    const cmd = toAwkString(await withDefenseContext(ctx, "getline command expression", () => evalExpr(ctx, cmdExpr)));
...
async function evalGetlineFromFile(ctx, variable, fileExpr) {
    ...
    const filename = toAwkString(await withDefenseContext(ctx, "getline filename evaluation", () => evalExpr(ctx, fileExpr)));
