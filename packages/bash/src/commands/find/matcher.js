 ...
 */
export function evaluateExpressionWithPrune(expr, ctx) {
    switch (expr.type) {
        case "name": {
 ...
 */
export function expressionNeedsStatMetadata(expr) {
    if (!expr)
        return false;
    switch (expr.type) {
        ...
        case "and":
        case "or":
            return (expressionNeedsStatMetadata(expr.left) ||
 ...
 */
export function expressionNeedsEmptyCheck(expr) {
    if (!expr)
        return false;
    switch (expr.type) {
        case "empty":
        ...
        case "and":
        case "or":
            return (expressionNeedsEmptyCheck(expr.left) ||
 ...
 */
export function extractPathPruningHints(expr) {
    const hint = {
...
export function collectNewerRefs(expr) {
    const refs = [];
    const collect = (e) => {
        if (!e)
            return;
 ...
 */
export function evaluateSimpleExpression(expr, name, relativePath, isFile, isDirectory) {
    switch (expr.type) {
        case "name": {
            // Fast path: check extension before full glob match for patterns like "*.json"
            const pattern = expr.pattern;
 ...
 */
export function expressionHasPrune(expr) {
    if (!expr)
        return false;
    switch (expr.type) {
        case "prune":
...
function canEvaluateExpressionEarly(expr) {
    switch (expr.type) {
        ...
        case "and":
        case "or":
            return (canEvaluateExpressionEarly(expr.left) &&
 ...
 */
export function evaluateForEarlyPrune(expr, ctx) {
