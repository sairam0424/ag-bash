 ...
 */
const jsExecAsyncContext = new AsyncLocalStorage();
 ...
 */
function normalizeJsWorkerMessage(msg, expectedProtocolToken) {
...
function getOrCreateWorker() {
    ...
    worker.on("message", (msg) => {
        ...
        if (currentExecution) {
            const result = normalizeJsWorkerMessage(msg, currentExecution.input.protocolToken);
 ...
 */
async function executeJS(jsCode, ctx, scriptPath, scriptArgs = [], bootstrapCode, isModule, stripTypes) {
    ...
    async execute(args, ctx) {
        ...
        if (parsed.code !== null) {
            ...
            try {
                ...
                return {
                    stdout: "",
                    stderr: `js-exec: can't open file '${parsed.scriptFile}': ${sanitizeErrorMessage(e.message)}\n`,
                    exitCode: 2,
                };
            }
        }
        ...
        return executeJS(jsCode, ctx, scriptPath, parsed.scriptArgs, bootstrapCode, isModule, stripTypes);
    },
};
