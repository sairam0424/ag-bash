...
const argDefs = {
    ...
    classifyFiles: { short: "F", long: "classify", type: "boolean" },
    ...
    async execute(args, ctx) {
        ...
        for (let i = 0; i < paths.length; i++) {
            ...
            if (path.includes("*") || path.includes("?") || path.includes("[")) {
                const result = await listGlob(path, ctx, showAll, showAlmostAll, longFormat, reverse, humanReadable, sortBySize, classifyFiles);
                ...
                const result = await listPath(path, ctx, showAll, showAlmostAll, longFormat, recursive, paths.length > 1, reverse, humanReadable, sortBySize, classifyFiles);
                stdout += result.stdout;
                stderr += result.stderr;
                if (result.exitCode !== 0)
                    exitCode = result.exitCode;
...
async function listGlob(pattern, ctx, showAll, showAlmostAll, longFormat, reverse = false, humanReadable = false, sortBySize = false, classifyFiles = false) {
    ...
    if (longFormat) {
        ...
        for (const match of matches) {
            ...
            try {
                ...
                const mtime = stat.mtime ?? new Date(0);
                const dateStr = formatDate(mtime);
                lines.push(`${mode} 1 user user ${sizeStr} ${dateStr} ${match}${suffix}`);
            }
            catch {
...
async function listPath(path, ctx, showAll, showAlmostAll, longFormat, recursive, showHeader, reverse = false, humanReadable = false, sortBySize = false, classifyFiles = false, _isSubdir = false) {
    const showHidden = showAll || showAlmostAll;
    const fullPath = ctx.fs.resolvePath(ctx.cwd, path);
    try {
        const stat = await ctx.fs.stat(fullPath);
        ...
        if (recursive) {
            ...
            for (let i = 0; i < dirEntries.length; i += DEFAULT_BATCH_SIZE) {
                ...
                const batchResults = await Promise.all(batch.map(async (dir) => {
                    ...
                    const result = await listPath(subPath, ctx, showAll, showAlmostAll, longFormat, recursive, false, reverse, humanReadable, sortBySize, classifyFiles, true);
