 ...
 */
export function evalControlBuiltin(value, name, args, ctx, evaluate, evaluateWithPartialResults, isTruthy, ExecutionLimitError) {
    switch (name) {
        case "first":
            if (args.length > 0) {
        ...
        case "range": {
            ...
            const rangeExceeded = (result) => result.length >= maxRange;
        ...
        case "until": {
            ...
            throw new ExecutionLimitError(`jq until: too many iterations (${maxIterations}), increase executionLimits.maxJqIterations`, "iterations");
        }
        case "while": {
            if (args.length < 2)
                return [value];
            ...
            if (results.length >= maxIterations) {
                throw new ExecutionLimitError(`jq while: too many iterations (${maxIterations}), increase executionLimits.maxJqIterations`, "iterations");
            }
            return results;
        }
        case "repeat": {
            ...
            if (results.length >= maxIterations) {
                throw new ExecutionLimitError(`jq repeat: too many iterations (${maxIterations}), increase executionLimits.maxJqIterations`, "iterations");
            }
            return results;
        }
        default:
