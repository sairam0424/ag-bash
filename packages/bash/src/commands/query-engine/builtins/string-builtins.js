 ...
 */
export function evalStringBuiltin(value, name, args, ctx, evaluate) {
    switch (name) {
        case "join": {
        ...
        case "splits": {
            ...
            try {
                const flags = args.length > 1 ? String(evaluate(value, args[1], ctx)[0]) : "g";
        ...
        case "scan": {
            ...
            const pattern = String(patterns[0]);
            try {
                const flags = args.length > 1 ? String(evaluate(value, args[1], ctx)[0]) : "";
        ...
        case "sub": {
            ...
            try {
                const flags = args.length > 2 ? String(evaluate(value, args[2], ctx)[0]) : "";
        ...
        case "gsub": {
            ...
            const replacement = String(replacements[0]);
            try {
                const flags = args.length > 2 ? String(evaluate(value, args[2], ctx)[0]) : "g";
