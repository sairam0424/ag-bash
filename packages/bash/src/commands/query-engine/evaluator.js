...
import { evalArrayBuiltin, evalControlBuiltin, evalDateBuiltin, evalFormatBuiltin, evalIndexBuiltin, evalMathBuiltin, evalNavigationBuiltin, evalObjectBuiltin, evalPathBuiltin, evalSqlBuiltin, evalStringBuiltin, evalTypeBuiltin, } from "./builtins/index.js";
import { deletePath, setPath } from "./path-operations.js";
import { asQueryRecord, isSafeKey, nullPrototypeCopy, nullPrototypeMerge, safeHasOwn, safeSet, } from "./safe-object.js";
import { compare, compareJq, containsDeep, deepEqual, deepMerge, getValueDepth, isTruthy, } from "./value-operations.js";
 ...
 */
const SIMPLE_MATH_FUNCTIONS = new Map([
    ["floor", Math.floor],
    ["ceil", Math.ceil],
    ["round", Math.round],
    ["sqrt", Math.sqrt],
...
export function evaluate(value, ast, ctxOrOptions) {
    let ctx = ctxOrOptions && "vars" in ctxOrOptions
        ? ctxOrOptions
        : createContext(ctxOrOptions);
        ...
        case "Try": {
            try {
                ...
                if (ast.catch) {
                    ...
                    const errorVal = e instanceof JqError
                        ? e.value
                        : e instanceof Error
                            ? e.message
                            : String(e);
    ...
    function deleteAt(val, path) {
        switch (path.type) {
            ...
            case "Pipe": {
                ...
                // Helper to set a value at an AST path
                function setAt(obj, pathNode, newVal) {
                    switch (pathNode.type) {
                        case "Identity":
                            return newVal;
                        case "Field": {
...
function evalBinaryOp(value, op, left, right, ctx) {
    ...
    if (op === "//") {
        const leftVals = evaluate(value, left, ctx);
        const nonNull = leftVals.filter((v) => v !== null && v !== undefined && v !== false);
        if (nonNull.length > 0)
            return nonNull;
    ...
    switch (name) {
        ...
        default: {
            ...
            if (userFunc) {
                ...
                const baseFuncs = (userFunc.closure ?? ctx.funcs ?? new Map());
