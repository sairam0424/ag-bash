 ...
 */
export async function executeSearch(searchCtx) {
    ...
    const showFilename = !options.noFilename &&
        ...
        for (const entry of entries) {
            ...
            if (hasTypeInfo) {
                ...
                try {
                    ...
                    const stat = isSymlink && options.followSymlinks
                        ? await ctx.fs.stat(entryAbsolutePath)
                        : lstat;
                    isFile = stat.isFile;
                    isDirectory = stat.isDirectory;
    ...
    if (options.globs.length > 0) {
        ...
        if (positiveGlobs.length > 0) {
            ...
            for (const glob of positiveGlobs) {
                if (matchGlob(filename, glob, ignoreCase) ||
        ...
        for (const glob of negativeGlobs) {
            ...
            else if (matchGlob(filename, glob, ignoreCase) ||
                matchGlob(relativePath, glob, ignoreCase)) {
                return false;
            }
        }
    ...
    if (options.iglobs.length > 0) {
        ...
        if (positiveIglobs.length > 0) {
            ...
            for (const glob of positiveIglobs) {
                if (matchGlob(filename, glob, true) ||
        ...
        for (const glob of negativeIglobs) {
            ...
            else if (matchGlob(filename, glob, true) ||
                matchGlob(relativePath, glob, true)) {
                return false;
            }
        }
