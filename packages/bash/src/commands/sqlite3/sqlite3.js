...
function normalizeWorkerResult(result, expectedProtocolToken) {
    ...
    return {
        ...
        dbBuffer: raw.dbBuffer === undefined
            ? null
            : raw.dbBuffer,
        defenseStats: raw.defenseStats,
    };
}
async function executeInWorker(input, timeoutMs, requireDefenseContext) {
    // Try to use worker thread for timeout protection
    try {
        const workerPath = findWorkerPath();
        return await new Promise((resolve, reject) => {
            const worker = DefenseInDepthBox.runTrusted(() => _internals.createWorker(workerPath, input));
            const onTimeout = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker timeout callback", () => {
                worker.terminate();
                resolve({
                    success: false,
            ...
            const onMessage = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker message callback", (result) => {
                _clearTimeout(timeout);
                resolve(normalizeWorkerResult(result, input.protocolToken));
            });
            const onError = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker error callback", (err) => {
                _clearTimeout(timeout);
                reject(err);
            });
            const onExit = bindDefenseContextCallback(requireDefenseContext, "sqlite3", "worker exit callback", (code) => {
                _clearTimeout(timeout);
                if (code !== 0) {
                    resolve({
                        success: false,
...
export const sqlite3Command = {
    ...
    async execute(args, ctx) {
        ...
        let result;
        try {
            result = await executeInWorker(workerInput, timeoutMs, ctx.requireDefenseContext);
        }
        catch (e) {
            const message = sanitizeHostErrorMessage(e.message);
            return {
