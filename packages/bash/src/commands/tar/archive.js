...
async function decompressBzip2(data) {
    const decompressed = seekBzip.decode(Buffer.from(data));
 ...
 */
export async function createXzCompressedArchive(entries, options) {
    if (!options?.allowNativeCodecs) {
        throw new Error("xz compression is disabled by default (native codec risk). " +
 ...
 */
export async function parseXzCompressedArchive(data, options) {
    if (!options?.allowNativeCodecs) {
        return {
            entries: [],
            error: "xz decompression is disabled by default (native codec risk). " +
 ...
 */
export async function createZstdCompressedArchive(entries, options) {
    if (!options?.allowNativeCodecs) {
        throw new Error("zstd compression is disabled by default (native codec risk). " +
            "Pass { allowNativeCodecs: true } to opt in.");
    }
    const tarBuffer = await createArchive(entries);
 ...
 */
export async function parseZstdCompressedArchive(data, options) {
    if (!options?.allowNativeCodecs) {
        return {
            entries: [],
            error: "zstd decompression is disabled by default (native codec risk). " +
                "Pass { allowNativeCodecs: true } to opt in, or decompress the archive externally before extraction.",
        };
    }
    if (data.length > MAX_ARCHIVE_SIZE) {
