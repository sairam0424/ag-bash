...
import { createArchive, createBzip2CompressedArchive, createCompressedArchive, createXzCompressedArchive, createZstdCompressedArchive, isBzip2Compressed, isGzipCompressed, isXzCompressed, isZstdCompressed, parseArchive, parseBzip2CompressedArchive, parseCompressedArchive, parseXzCompressedArchive, parseZstdCompressedArchive, } from "./archive.js";
...
function matchesExclude(path, patterns) {
    ...
    for (const pattern of patterns) {
        ...
        if (createUserRegex(`^${regex}$`).test(path) ||
...
function matchesWildcard(name, pattern) {
    ...
    const basename = name.includes("/")
        ...
        : name;
    return (createUserRegex(`^${regex}$`).test(name) ||
        createUserRegex(`^${regex}$`).test(basename));
}
/**
...
async function updateTarArchive(ctx, options, files) {
    ...
    const existingEntries = parseResult.entries
    ...
    for (const entry of parseResult.entries) {
        ...
        if (normalizedSpecificFiles.length > 0) {
            ...
            if (options.wildcards) {
                ...
                matches = normalizedSpecificFiles.some((f) => matchesWildcard(safeName, f) ||
...
async function listTarArchive(ctx, options, specificFiles) {
    ...
    for (const entry of parseResult.entries) {
        ...
        if (specificFiles.length > 0) {
            let matches;
            if (options.wildcards) {
                matches = specificFiles.some((f) => matchesWildcard(name, f) ||
                    matchesWildcard(displayName, f) ||
                    name.startsWith(`${f}/`));
            }
            else {
