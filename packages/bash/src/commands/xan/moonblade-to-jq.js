...
const FUNCTION_MAP = {
    ...
    // Type functions
    type: "type",
    isnull: (args) => args.length === 0
        ? makeBinaryOp("==", { type: "Identity" }, { type: "Literal", value: null })
    ...
    coalesce: (args) => {
        ...
        return makeCond(condition, first, rest.length === 1
            ? rest[0]
            : FUNCTION_MAP.coalesce(rest));
    },
 ...
 * Transform moonblade AST to jq AST
 */
export function moonbladeToJq(expr, rowContext = true) {
    switch (expr.type) {
        case "int":
        case "float":
