 ...
 */
export async function cmdTranspose(args, ctx) {
 ...
 */
export async function cmdShuffle(args, ctx) {
    let seed = null;
    const fileArgs = [];
 ...
 */
export async function cmdFixlengths(args, ctx) {
    let targetLen = null;
    let defaultValue = "";
    const fileArgs = [];
    for (let i = 0; i < args.length; i++) {
 ...
 */
export async function cmdSplit(args, ctx) {
    let numParts = null;
    let partSize = null;
    let outputDir = ".";
    const fileArgs = [];
 ...
 *   -o, --output DIR  Output directory (default: current)
 */
export async function cmdPartition(args, ctx) {
    let column = "";
    let outputDir = ".";
    const fileArgs = [];
    for (let i = 0; i < args.length; i++) {
 ...
 *   FORMAT: json
 */
export async function cmdTo(args, ctx) {
    if (args.length === 0) {
        return {
            stdout: "",
 ...
 */
export async function cmdFrom(args, ctx) {
    let format = "";
    const fileArgs = [];
    for (let i = 0; i < args.length; i++) {
        const arg = args[i];
        ...
        if (Array.isArray(data[0])) {
            ...
            const csvData = rows.map((row) => {
                ...
                for (let i = 0; i < headers.length; i++) {
                    safeSetRow(obj, headers[i], row[i]);
                }
                return obj;
            });
            return {
