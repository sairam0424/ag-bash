...
export function parseInput(input, options) {
    ...
    switch (options.inputFormat) {
        ...
        case "csv":
            // SECURITY: sanitizeParsedData converts to null-prototype objects at boundary.
            return sanitizeParsedData(parseCsv(trimmed, options.csvDelimiter, options.csvHeader));
        case "toml":
            // SECURITY: sanitizeParsedData converts to null-prototype objects at boundary.
...
export function parseAllYamlDocuments(input) {
    ...
    return docs.map((doc) => sanitizeParsedData(doc.toJS({ maxAliasCount: 100 })));
}
/**
...
export function extractFrontMatter(input) {
    ...
    if (trimmed.startsWith("---")) {
        ...
        if (endMatch && endMatch.index !== undefined) {
            ...
            return {
                frontMatter: sanitizeParsedData(YAML.parse(yamlContent, { maxAliasCount: 100 })),
                content: remaining,
            };
        }
    }
