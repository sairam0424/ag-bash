...
const validInputFormats = ["yaml", "xml", "json", "ini", "csv", "toml"];
const validOutputFormats = ["yaml", "json", "xml", "ini", "csv", "toml"];
/**
 * Type guard to validate input format strings at runtime
 */
export function isValidInputFormat(value) {
  return typeof value === "string" && validInputFormats.includes(value);
}
/**
 * Type guard to validate output format strings at runtime
 */
export function isValidOutputFormat(value) {
  return typeof value === "string" && validOutputFormats.includes(value);
}
...
export function parseInput(input, options) {
  ...
  switch (options.inputFormat) {
    ...
    case "toml":
      // SECURITY: sanitizeParsedData converts to null-prototype objects at boundary.
      return sanitizeParsedData(TOML.parse(trimmed));
    default: {
...
export function extractFrontMatter(input) {
  ...
  if (trimmed.startsWith("+++")) {
    ...
    if (endMatch && endMatch.index !== undefined) {
      ...
      const remaining = trimmed.slice(endMatch.index + 3 + endMatch[0].length);
      return {
        frontMatter: sanitizeParsedData(TOML.parse(tomlContent)),
        content: remaining,
      };
    }
  }
