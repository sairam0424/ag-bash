...
export const yqCommand = {
    ...
    async execute(args, ctx) {
        ...
        const withDefenseContext = (phase, op) => awaitWithDefenseContext(ctx.requireDefenseContext, "yq", phase, op);
        if (hasHelpFlag(args))
        ...
        if (options.nullInput) {
            ...
            try {
                ...
                input = await withDefenseContext("file read", () => ctx.fs.readFile(resolvedFilePath));
            }
            catch (e) {
        ...
        try {
            ...
            if (options.inplace && filePath) {
                await withDefenseContext("in-place write", () => ctx.fs.writeFile(filePath, finalOutput));
