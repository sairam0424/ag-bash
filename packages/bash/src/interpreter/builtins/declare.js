...
import { isNameref, markNameref, markNamerefBound, markNamerefInvalid, resolveNameref, targetExists, unmarkNameref, } from "../helpers/nameref.js";
import { checkReadonlyError, markExported, markReadonly, unmarkExported, } from "../helpers/readonly.js";
import { OK, result, success } from "../helpers/result.js";
import { expandTildesInValue } from "../helpers/tilde.js";
import { parseArrayElements, parseAssocArrayLiteral, } from "./declare-array-parsing.js";
import { listAllVariables, listAssociativeArrays, listIndexedArrays, printAllVariables, printSpecificVariables, } from "./declare-print.js";
import { markLocalVarDepth, parseAssignment, setVariable, } from "./variable-assignment.js";
/**
 ...
 */
export function isInteger(ctx, name) {
 ...
 */
export function applyCaseTransform(ctx, name, value) {
...
export async function handleDeclare(ctx, args) {
    // Parse flags
    ...
    for (const arg of processedArgs) {
        ...
        if (arrayMatch && !removeArray) {
            ...
            if (declareAssoc && content.includes("[")) {
                ...
                for (let i = 0; i < elements.length; i += 2) {
                    ...
                    const value = i + 1 < elements.length
        ...
        if (appendMatch) {
            ...
            const existingIndices = getArrayIndices(ctx, name);
            const isArray = existingIndices.length > 0 || ctx.state.associativeArrays?.has(name);
 ...
 */
export async function handleReadonly(ctx, args) {
    // Parse flags
