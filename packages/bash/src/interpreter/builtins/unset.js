...
import { clearLocalVarDepth, getLocalVarDepth, popLocalVarStack, } from "./variable-assignment.js";
/**
...
function isQuotedStringIndex(indexExpr) {
    ...
    return ((indexExpr.startsWith("'") && indexExpr.endsWith("'")) ||
...
export async function handleUnset(ctx, args) {
    let mode = "both"; // Default: unset both var and func
    let stderr = "";
    let exitCode = 0;
    for (const arg of args) {
        ...
        if (mode === "variable") {
            ...
            if (arrayMatchVar) {
                ...
                const isScalar = (ctx.state.env.has(arrayName) || isDeclaredButUnset) &&
                    !isIndexedArray &&
                    !isAssoc;
                if (isScalar) {
