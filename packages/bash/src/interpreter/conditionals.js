...
import { escapeRegexChars, expandWord, expandWordForPattern, expandWordForRegex, } from "./expansion.js";
import { clearArray } from "./helpers/array.js";
import { evaluateBinaryFileTest, evaluateFileTest, isBinaryFileTestOperator, isFileTestOperator, } from "./helpers/file-tests.js";
...
export async function evaluateConditional(ctx, expr) {
    switch (expr.type) {
        case "CondBinary": {
            ...
            const isRhsQuoted = expr.right.parts.length > 0 &&
                expr.right.parts.every((p) => p.type === "SingleQuoted" ||
                    p.type === "DoubleQuoted" ||
        ...
        case "CondNot": {
            ...
            if (ctx.state.shoptOptions.extglob) {
                if (expr.operand.type === "CondGroup" &&
...
export async function evaluateTestArgs(ctx, args) {
    if (args.length === 0) {
        return execResult("", "", 1);
    }
    if (args.length === 1) {
    ...
    if (args.length === 3) {
        ...
        if (left === "!") {
            ...
            return execResult("", negResult.stderr, negResult.exitCode === 0
                ? 1
...
export function matchPattern(value, pattern, nocasematch = false, extglob = false) {
            ...
            if (closeIdx !== -1) {
                ...
                if (char === "@") {
                    ...
                    if (hasMorePattern) {
                        ...
                        const allSameLength = lengths.every((l) => l !== null) &&
