...
import { escapeGlobChars, expandWord, expandWordWithGlob, isWordFullyQuoted, } from "./expansion.js";
...
export async function executeIf(ctx, node) {
    let stdout = "";
    let stderr = "";
...
export async function executeFor(ctx, node) {
...
export async function executeCStyleFor(ctx, node) {
...
export async function executeWhile(ctx, node, stdin = "") {
    let stdout = "";
    let stderr = "";
    let exitCode = 0;
    let iterations = 0;
...
export async function executeUntil(ctx, node) {
    let stdout = "";
    let stderr = "";
    let exitCode = 0;
    let iterations = 0;
...
export async function executeCase(ctx, node) {
    ...
    for (let i = 0; i < node.items.length; i++) {
        ...
        if (matched) {
            const bodyResult = await executeStatements(ctx, item.body, stdout, stderr);
