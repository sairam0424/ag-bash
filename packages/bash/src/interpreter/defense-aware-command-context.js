...
function wrapFileSystem(fs, requireDefenseContext, component) {
    const wrappedFs = {
        ...
        exists: wrapFunction(fs.exists.bind(fs), requireDefenseContext, component, "fs.exists"),
        stat: wrapFunction(fs.stat.bind(fs), requireDefenseContext, component, "fs.stat"),
        mkdir: wrapFunction(fs.mkdir.bind(fs), requireDefenseContext, component, "fs.mkdir"),
        readdir: wrapFunction(fs.readdir.bind(fs), requireDefenseContext, component, "fs.readdir"),
        ...
        resolvePath: wrapFunction(fs.resolvePath.bind(fs), requireDefenseContext, component, "fs.resolvePath"),
        getAllPaths: wrapFunction(fs.getAllPaths.bind(fs), requireDefenseContext, component, "fs.getAllPaths"),
        chmod: wrapFunction(fs.chmod.bind(fs), requireDefenseContext, component, "fs.chmod"),
        symlink: wrapFunction(fs.symlink.bind(fs), requireDefenseContext, component, "fs.symlink"),
        link: wrapFunction(fs.link.bind(fs), requireDefenseContext, component, "fs.link"),
        readlink: wrapFunction(fs.readlink.bind(fs), requireDefenseContext, component, "fs.readlink"),
        lstat: wrapFunction(fs.lstat.bind(fs), requireDefenseContext, component, "fs.lstat"),
        realpath: wrapFunction(fs.realpath.bind(fs), requireDefenseContext, component, "fs.realpath"),
        utimes: wrapFunction(fs.utimes.bind(fs), requireDefenseContext, component, "fs.utimes"),
    };
    if (fs.readdirWithFileTypes) {
        wrappedFs.readdirWithFileTypes = wrapFunction(fs.readdirWithFileTypes.bind(fs), requireDefenseContext, component, "fs.readdirWithFileTypes");
    }
    return wrappedFs;
}
/**
 ...
 */
export function createDefenseAwareCommandContext(ctx, commandName) {
