...
import { BadSubstitutionError, ExecutionLimitError, ExitError, } from "./errors.js";
/**
...
import { expandDollarVarsInArithText, expandSubscriptForAssocArray, } from "./expansion/arith-text-expansion.js";
...
import { escapeGlobChars, escapeRegexChars, hasGlobPattern, } from "./expansion/glob-escape.js";
import { computeIsEmpty, handleArrayKeys, handleAssignDefault, handleCaseModification, handleDefaultValue, handleErrorIfUnset, handleIndirection, handleLength, handlePatternRemoval, handlePatternReplacement, handleSubstring, handleTransform, handleUseAlternative, handleVarNamePrefix, } from "./expansion/parameter-ops.js";
import { expandVariablesInPattern, expandVariablesInPatternAsync, patternHasCommandSubstitution, } from "./expansion/pattern-expansion.js";
import { applyTildeExpansion } from "./expansion/tilde.js";
import { getVariable, isVariableSet } from "./expansion/variable.js";
import { expandWordWithGlobImpl, } from "./expansion/word-glob-expansion.js";
import { smartWordSplit } from "./expansion/word-split.js";
import { buildIfsCharClassPattern, getIfs, isIfsEmpty, splitByIfsForExpansion, } from "./helpers/ifs.js";
 ...
 */
export function isWordFullyQuoted(word) {
...
export async function expandWord(ctx, word) {
 ...
 */
export async function expandWordForRegex(ctx, word) {
    const parts = [];
    for (const part of word.parts) {
 ...
 */
export async function expandWordForPattern(ctx, word) {
    const parts = [];
    for (const part of word.parts) {
        if (part.type === "Escaped") {
...
async function expandBracesInPartsAsync(ctx, parts, operationCounter = { count: 0 }) {
    ...
    for (const part of parts) {
        if (part.type === "BraceExpansion") {
            ...
            if (newSize > ctx.limits.maxBraceExpansionResults ||
...
export async function expandWordWithGlob(ctx, word) {
...
export function hasQuotedMultiValueAt(ctx, word) {
        ...
        for (const part of parts) {
            if (part.type === "DoubleQuoted") {
                ...
                for (const innerPart of part.parts) {
                    if (innerPart.type === "ParameterExpansion" &&
                        innerPart.parameter === "@" &&
 ...
 */
export async function expandRedirectTarget(ctx, word) {
