 ...
 */
async function buildPatternRegex(ctx, pattern, expandWordPartsAsync, expandPart) {
    let regex = "";
 ...
 */
export async function handleArrayPatternReplacement(ctx, wordParts, expandWordPartsAsync, expandPart) {
    ...
    if (operation.pattern) {
        regex = await buildPatternRegex(ctx, operation.pattern, expandWordPartsAsync, expandPart);
    }
 ...
 * Returns null if this handler doesn't apply.
 */
export async function handleArrayPatternRemoval(ctx, wordParts, expandWordPartsAsync, expandPart) {
    if (wordParts.length !== 1 || wordParts[0].type !== "DoubleQuoted") {
        return null;
    }
    const dqPart = wordParts[0];
