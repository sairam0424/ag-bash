 ...
 */
export async function handleArraySlicing(ctx, wordParts, evaluateArithmetic) {
    ...
    if (offset < 0) {
        ...
        if (elements.length > 0) {
            ...
            if (startIdx < 0)
                startIdx = elements.length; // All elements have smaller index
        }
    }
    else {
 ...
 */
export function handleArrayTransform(ctx, wordParts) {
    if (wordParts.length !== 1 || wordParts[0].type !== "DoubleQuoted") {
        return null;
    }
    const dqPart = wordParts[0];
