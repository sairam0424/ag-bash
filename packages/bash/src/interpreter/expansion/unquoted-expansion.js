...
import { getIfs, getIfsSeparator, isIfsEmpty, isIfsWhitespaceOnly, splitByIfsForExpansion, } from "../helpers/ifs.js";
import { escapeRegex } from "../helpers/regex.js";
import { hasGlobPattern } from "./glob-escape.js";
import { patternToRegex } from "./pattern.js";
import { applyPatternRemoval, getVarNamesWithPrefix, } from "./pattern-removal.js";
 ...
 */
export async function handleUnquotedArrayPatternReplacement(ctx, wordParts, expandWordPartsAsync, expandPart) {
 ...
 */
export async function handleUnquotedArrayPatternRemoval(ctx, wordParts, expandWordPartsAsync, expandPart) {
 ...
 */
export async function handleUnquotedPositionalPatternRemoval(ctx, wordParts, expandWordPartsAsync, expandPart) {
 ...
 */
export async function handleUnquotedPositionalSlicing(ctx, wordParts, evaluateArithmetic, expandPart) {
 ...
 */
export async function handleUnquotedSimplePositional(ctx, wordParts) {
 ...
 */
export async function handleUnquotedSimpleArray(ctx, wordParts) {
    if (wordParts.length !== 1 ||
        wordParts[0].type !== "ParameterExpansion" ||
        wordParts[0].operation) {
        return null;
 ...
 */
export function handleUnquotedVarNamePrefix(ctx, wordParts) {
    if (wordParts.length !== 1 ||
        wordParts[0].type !== "ParameterExpansion" ||
 ...
 */
export function handleUnquotedArrayKeys(ctx, wordParts) {
    if (wordParts.length !== 1 ||
        wordParts[0].type !== "ParameterExpansion" ||
 ...
 */
export async function handleUnquotedPositionalWithPrefixSuffix(ctx, wordParts, expandPart) {
