...
import { globPatternHasVarRef, isOperationWordEntirelyQuoted, } from "./analysis.js";
/**
...
async function hasMixedQuotedDefaultValue(ctx, part) {
    ...
    const hasUnquotedParts = opWordParts.some((p) => p.type === "Literal" ||
 ...
 */
export async function smartWordSplit(ctx, wordParts, ifsChars, _ifsPattern, expandPartFn) {
    ...
    if (wordParts.length === 1 && wordParts[0].type === "ParameterExpansion") {
        ...
        if (opWordParts && opWordParts.length > 0) {
            ...
            const hasMixedParts = opWordParts.length > 1 &&
                opWordParts.some((p) => p.type === "DoubleQuoted" || p.type === "SingleQuoted") &&
                opWordParts.some((p) => p.type === "Literal" ||
                    p.type === "ParameterExpansion" ||
                    p.type === "CommandSubstitution" ||
                    p.type === "ArithmeticExpansion");
            if (hasMixedParts) {
