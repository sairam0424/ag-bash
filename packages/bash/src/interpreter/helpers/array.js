 ...
 */
export function getArrayIndices(ctx, arrayName) {
 ...
 */
export function clearArray(ctx, arrayName) {
    const prefix = `${arrayName}_`;
    for (const key of ctx.state.env.keys()) {
        if (key.startsWith(prefix)) {
 ...
 */
export function getAssocArrayKeys(ctx, arrayName) {
    const prefix = `${arrayName}_`;
...
export function parseKeyedElementFromWord(word) {
 ...
 */
export function wordToLiteralString(word) {
    let result = "";
