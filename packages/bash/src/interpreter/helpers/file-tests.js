...
export function isFileTestOperator(op) {
 ...
 */
export async function evaluateFileTest(ctx, operator, operand) {
 ...
 */
const BINARY_FILE_TEST_OPERATORS = ["-nt", "-ot", "-ef"];
export function isBinaryFileTestOperator(op) {
 ...
 */
export async function evaluateBinaryFileTest(ctx, operator, left, right) {
    ...
    switch (operator) {
        ...
        case "-ef": {
            ...
            try {
                ...
                if (!(await ctx.fs.exists(leftPath)) ||
