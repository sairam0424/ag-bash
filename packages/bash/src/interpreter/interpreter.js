...
import { checkFdLimit, failure, OK, result, testResult, throwExecutionLimit, } from "./helpers/result.js";
import { isPosixSpecialBuiltin } from "./helpers/shell-constants.js";
import { isWordLiteralMatch, parseRwFdContent, } from "./helpers/word-matching.js";
...
export class Interpreter {
    ...
    async executeScript(node) {
        ...
        const appendOutput = (nextStdout, nextStderr) => {
            if (stdout.length + stderr.length + nextStdout.length + nextStderr.length >
                maxOutputSize) {
    ...
    async executeSimpleCommandInner(node, stdin) {
            ...
            if (redir.operator === "<&" && redir.target.type === "Word") {
                ...
                if (!Number.isNaN(sourceFd) && this.ctx.state.fileDescriptors) {
                    ...
                    if (fdContent !== undefined) {
                        ...
                        else if (fdContent.startsWith("__file__:") ||
        ...
        if (!commandName) {
            const isOnlyExpansions = node.name.parts.every((p) => p.type === "CommandSubstitution" ||
    ...
    async executeSubshell(node, stdin = "") {
        return executeSubshellHelper(this.ctx, node, stdin, (stmt) => this.executeStatement(stmt));
    }
    async executeGroup(node, stdin = "") {
        return executeGroupHelper(this.ctx, node, stdin, (stmt) => this.executeStatement(stmt));
    }
