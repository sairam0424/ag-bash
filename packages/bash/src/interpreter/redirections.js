 ...
 */
import { expandRedirectTarget, expandWord, hasQuotedMultiValueAt, } from "./expansion.js";
 ...
 */
export async function preExpandRedirectTargets(ctx, redirections) {
 ...
 */
export async function processFdVariableRedirections(ctx, redirections) {
 ...
 */
export async function preOpenOutputRedirects(ctx, redirections) {
    for (const redir of redirections) {
        if (redir.target.type === "HereDoc") {
            continue;
        }
        ...
        try {
            ...
            if (ctx.state.options.noclobber &&
                ...
                !stat.isDirectory &&
                target !== "/dev/null") {
                return makeResult("", `bash: ${target}: cannot overwrite existing file\n`, 1);
            }
        }
        catch {
            // File doesn't exist, that's ok - we'll create it
