...
describe('Nexus Prime Integration (MCP & Orchestration)', () => {
  ...
  describe('Pillar 4: Orchestration (Sub-agents)', () => {
    ...
    it('should handle invalid sub-agent ID', async () => {
      ...
      expect(result.stderr).toContain('Wait failed: Agent non_existent not found');
    });
    ...
    it('should handle duplicate sub-agent ID', async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain('Spawn failed: Agent with ID dup already exists');
    });
  });
  ...
  describe('Pillar 3: MCP Integration', () => {
    it('should register MCP tools in the toolbox after connection', async () => {
      const mockConn = {
        ...
        tools: [{ name: 'mcp_test_tool', description: 'test', inputSchema: {} }]
      };
      mockClient.connectStdio.mockResolvedValueOnce(mockConn);
      mockClient.listConnections.mockReturnValue([mockConn]);

      const result = await bash.exec('ag-mcp connect stdio test-server node server.js');
      expect(result.exitCode).toBe(0);
      ...
      const hasTool = tools.some(t => t.name === 'mcp_test-server_mcp_test_tool');
    ...
    it('should handle connection errors gracefully', async () => {
      ...
      vi.mocked(client.connectStdio).mockRejectedValueOnce(new Error('Connection failed'));

      const result = await bash.exec('ag-mcp connect stdio fail-server node server.js');
      expect(result.exitCode).toBe(1);
  ...
  describe('Pillar 2: Intelligent Workflows', () => {
    it('should find symbols across the workspace', async () => {
      ...
      await bash.fs.writeFile('/test.sh', 'function my_test_bash_func() {\n  echo 1\n}');
