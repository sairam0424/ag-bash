...
function parseParameterOperation(p, value, start, _paramName, quoted = false) {
    ...
    if (char === ":") {
        ...
        if ("-=?+".includes(op)) {
            ...
            const wordParts = parseWordParts(p, wordStr, false, false, true, // isAssignment=true for tilde expansion after : in default values
            false, quoted, false, // noBraceExpansion
            false, // regexPattern
            true);
    ...
    if (char === "/") {
        ...
        if (value[patternEnd] === "/") {
            ...
            const replaceEnd = WordParser.findParameterOperationEnd(p, value, replaceStart);
...
export function parseWordParts(p, value, quoted = false, singleQuoted = false, isAssignment = false, hereDoc = false, 
...
inParameterExpansion = false) {
    ...
    while (i < value.length) {
        ...
        if (char === "{" && !isAssignment && !noBraceExpansion && !hereDoc) {
            const braceResult = WordParser.tryParseBraceExpansion(p, value, i, parseWordParts);
            if (braceResult) {
                flushLiteral();
