...
export class TreeSitterToAst {
  ...
  private convertSimpleCommand(node: Node): CommandNode {
    ...
    for (const child of node.namedChildren) {
      ...
      } else if (child.type === 'file_redirect' || child.type === 'heredoc_redirect' || child.type === 'herestring_redirect') {
        redirections.push(this.convertRedirection(child));
      } else if (child.type === 'command_name') {
        name = this.convertWord(child);
      } else if (child.type === 'word' || child.type === 'string' || child.type === 'raw_string' || child.type === 'concatenation') {
  ...
  private convertAssignment(node: Node): AssignmentNode {
    ...
    const valueNode = node.childForFieldName('value') || (node.namedChild(1)?.type === '=' ? node.namedChild(2) : node.namedChild(1));
  ...
  private convertRedirection(node: Node): RedirectionNode {
    ...
    const destNode = node.childForFieldName('destination') || node.childForFieldName('body');
  ...
  private convertWord(node: Node): WordNode {
    ...
    if (node.type === 'word' || node.type === 'command_name') {
        if (node.namedChildCount === 0) {
            parts.push({ type: "Literal", value: node.text, line: node.startPosition.row + 1 });
        } else {
            for (const child of node.namedChildren) {
    ...
    switch (node.type) {
      ...
      case 'command_substitution':
        return {
          ...
          body: { type: "Script", statements: this.convertList(node), line: node.startPosition.row + 1 },
          legacy: node.text.startsWith('`'),
          line: node.startPosition.row + 1
        };
      case 'word':
        return { type: "Literal", value: node.text, line: node.startPosition.row + 1 };
      default:
        return { type: "Literal", value: node.text, line: node.startPosition.row + 1 };
    }
  }
  ...
  private convertString(node: Node): WordPart {
    ...
    if (parts.length === 0 && node.text.length > 2) {
        parts.push({ type: "Literal", value: node.text.slice(1, -1), line: node.startPosition.row + 1 });
    }
  ...
  private convertIfStatement(node: Node): CompoundCommandNode {
    ...
    const conditionNode = node.childForFieldName('condition') || node.namedChild(0);
    const consequentNode = node.childForFieldName('consequent') || node.namedChild(1);
  ...
  private convertWhileUntilStatement(node: Node, type: "While" | "Until"): CommandNode | null {
    ...
    for (const child of node.namedChildren) {
        if (child.type === 'case_item') {
            ...
            for (const p of child.children) {
                if (p.isNamed && (p.type === 'word' || p.type === 'string' || p.type === 'concatenation')) {
