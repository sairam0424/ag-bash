...
var Tree = class _Tree {
  ...
  rootNodeWithOffset(offsetBytes, offsetExtent) {
    const address = _TRANSFER_BUFFER + SIZE_OF_NODE;
  ...
  getChangedRanges(other) {
    ...
    const count = C.getValue(_TRANSFER_BUFFER, "i32");
    const buffer = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
...
var TreeCursor = class _TreeCursor {
  ...
  get nodeText() {
    ...
    const startPosition = unmarshalPoint(_TRANSFER_BUFFER);
  ...
  get startPosition() {
    marshalTreeCursor(this);
    C._ts_tree_cursor_start_position_wasm(this.tree[0]);
    return unmarshalPoint(_TRANSFER_BUFFER);
  }
  ...
  gotoFirstChildForIndex(goalIndex) {
    marshalTreeCursor(this);
    C.setValue(_TRANSFER_BUFFER + SIZE_OF_CURSOR, goalIndex, "i32");
  ...
  gotoFirstChildForPosition(goalPosition) {
    marshalTreeCursor(this);
    marshalPoint(_TRANSFER_BUFFER + SIZE_OF_CURSOR, goalPosition);
  ...
  reset(node) {
    marshalNode(node);
    marshalTreeCursor(this, _TRANSFER_BUFFER + SIZE_OF_NODE);
   ...
   */
  resetTo(cursor) {
    marshalTreeCursor(this, _TRANSFER_BUFFER);
    marshalTreeCursor(cursor, _TRANSFER_BUFFER + SIZE_OF_CURSOR);
...
var Node = class {
  ...
  descendantsOfType(types, startPosition = ZERO_POINT, endPosition = ZERO_POINT) {
    ...
    const descendantCount = C.getValue(_TRANSFER_BUFFER, "i32");
    const descendantAddress = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
...
function marshalNode(node, index = 0) {
  let address = _TRANSFER_BUFFER + index * SIZE_OF_NODE;
...
function unmarshalNode(tree, address = _TRANSFER_BUFFER) {
...
function marshalTreeCursor(cursor, address = _TRANSFER_BUFFER) {
...
function unmarshalTreeCursor(cursor) {
  cursor[0] = C.getValue(_TRANSFER_BUFFER + 0 * SIZE_OF_INT, "i32");
  cursor[1] = C.getValue(_TRANSFER_BUFFER + 1 * SIZE_OF_INT, "i32");
  cursor[2] = C.getValue(_TRANSFER_BUFFER + 2 * SIZE_OF_INT, "i32");
  cursor[3] = C.getValue(_TRANSFER_BUFFER + 3 * SIZE_OF_INT, "i32");
}
...
function marshalEdit(edit, address = _TRANSFER_BUFFER) {
...
var Language = class _Language {
  ...
  */
  get metadata() {
    ...
    const length = C.getValue(_TRANSFER_BUFFER, "i32");
    if (length === 0) return null;
    return unmarshalLanguageMetadata(_TRANSFER_BUFFER + SIZE_OF_INT);
  }
  /**
...
// src/parser.ts
let _TRANSFER_BUFFER;
let _LANGUAGE_VERSION;
let _MIN_COMPATIBLE_VERSION;
var Parser = class {
  static {
    __name(this, "Parser");
  }
  ...
  static async init(moduleOptions) {
    ...
    _TRANSFER_BUFFER = C._ts_init();
    _LANGUAGE_VERSION = C.getValue(_TRANSFER_BUFFER, "i32");
    _MIN_COMPATIBLE_VERSION = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
  }
  /**
   * Create a new parser.
   */
  ...
  initialize() {
    ...
    this[0] = C.getValue(_TRANSFER_BUFFER, "i32");
    this[1] = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
  }
   ...
   * {@link _LANGUAGE_VERSION} and {@link _MIN_COMPATIBLE_VERSION} constants.
   */
  setLanguage(language) {
    ...
    if (!language) {
      ...
      if (version < _MIN_COMPATIBLE_VERSION || _LANGUAGE_VERSION < version) {
        throw new Error(
          `Incompatible language version ${version}. Compatibility range ${_MIN_COMPATIBLE_VERSION} through ${_LANGUAGE_VERSION}.`
        );
      }
      this.language = language;
    } else {
...
var Query = class {
  ...
  constructor(language, source) {
    ...
    const address = C._ts_query_new(
      ...
      sourceLength,
      _TRANSFER_BUFFER,
      _TRANSFER_BUFFER + SIZE_OF_INT
    );
    if (!address) {
      const errorId = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
      const errorByte = C.getValue(_TRANSFER_BUFFER, "i32");
    ...
    for (let i2 = 0; i2 < captureCount; i2++) {
      ...
      const nameLength = C.getValue(_TRANSFER_BUFFER, "i32");
    ...
    for (let i2 = 0; i2 < patternCount; i2++) {
      const predicatesAddress = C._ts_query_predicates_for_pattern(address, i2, _TRANSFER_BUFFER);
      const stepCount = C.getValue(_TRANSFER_BUFFER, "i32");
      predicates[i2] = [];
      textPredicates[i2] = [];
  ...
  matches(node, options = {}) {
    ...
    const rawCount = C.getValue(_TRANSFER_BUFFER, "i32");
    const startAddress = C.getValue(_TRANSFER_BUFFER + SIZE_OF_INT, "i32");
    const didExceedMatchLimit = C.getValue(_TRANSFER_BUFFER + 2 * SIZE_OF_INT, "i32");
...
export {
  CaptureQuantifier,
  Edit,
  _LANGUAGE_VERSION as LANGUAGE_VERSION,
  Language,
  LookaheadIterator,
  _MIN_COMPATIBLE_VERSION as MIN_COMPATIBLE_VERSION,
  Node,
  Parser,
  Query,
  Tree,
