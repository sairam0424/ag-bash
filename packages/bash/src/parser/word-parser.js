...
export function findTildeEnd(_p, value, start) {
  let i = start + 1;
...
export function findMatchingBracket(_p, value, start, open, close) {
  let depth = 1;
  let i = start + 1;
...
export function findParameterOperationEnd(_p, value, start) {
  let i = start;
  let depth = 1;
  while (i < value.length && depth > 0) {
    const char = value[i];
...
export function findPatternEnd(_p, value, start) {
  let i = start;
...
export function parseGlobPattern(_p, value, start) {
  let i = start;
  let pattern = "";
  while (i < value.length) {
    const char = value[i];
...
export function parseAnsiCQuoted(_p, value, start) {
  let result = "";
  let i = start;
  while (i < value.length && value[i] !== "'") {
    const char = value[i];
...
export function parseArithExprFromString(p, str) {
...
export function tryParseBraceExpansion(p, value, start, parseWordPartsFn) {
  // Find matching }
 ...
 */
export function wordToString(_p, word) {
  let result = "";
...
export function tokenToRedirectOp(_p, type) {
  const map = {
    [TokenType.LESS]: "<",
    [TokenType.GREAT]: ">",
    [TokenType.DGREAT]: ">>",
