...
const executionContext = !IS_BROWSER && AsyncLocalStorageClass
...
export class DefenseInDepthBox {
     ...
     */
    static trustedExecutionDepth = new Map();
    config;
    refCount = 0;
    patchFailures = [];
    ...
    static getInstance(config) {
        ...
        if (!DefenseInDepthBox.instance) {
            ...
            if (resolved.enabled !== active.enabled ||
                resolved.auditMode !== active.auditMode) {
                throw new Error(`DefenseInDepthBox config conflict: requested {enabled: ${resolved.enabled}, auditMode: ${resolved.auditMode}} ` +
    ...
    static enterTrustedScope(executionId) {
        const current = DefenseInDepthBox.trustedExecutionDepth.get(executionId) ?? 0;
    ...
    static bindCurrentContext(fn) {
        ...
        const executionId = current?.sandboxActive === true
            ? current.executionId
    ...
    applyPatches() {
        ...
        const skipInMainThread = new Set([
            "process_send",
            "process_channel",
    ...
    protectPromiseThen() {
        ...
        try {
             ...
             */
            function runGuardedCallback(...args) {
            ...
            Object.defineProperty(Promise.prototype, "then", {
                value: function patchedThen(onFulfilled, onRejected) {
                    ...
                    const store = executionContext.getStore();
                    const executionId = store?.sandboxActive === true && store.trusted !== true
                        ? store.executionId
                        : undefined;
                    if (!executionId) {
                        return Reflect.apply(originalThen, this, [onFulfilled, onRejected]);
    ...
    protectDynamicImport() {
        if (IS_BROWSER || DefenseInDepthBox.importHooksRegistered)
            return;
        try {
            const box = this;
            // eslint-disable-next-line @typescript-eslint/no-require-imports
            ...
            const shouldRecordAuditViolation = () => {
                const store = executionContext?.getStore();
                return (box.config.auditMode === true &&
