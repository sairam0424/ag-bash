...
describe("worker protocol runtime desync probes", () => {
  ...
  it("sqlite3 output shaped like worker control payload is treated as query data", {
    timeout: 60000,
  }, async () => {
    const bash = new Bash();
    const result = await bash.exec(
  ...
  it("sqlite3 timed-out worker query does not execute later and mutate database", {
    timeout: 60000,
  }, async () => {
