/**
 * Grep spec test runner - executes parsed grep tests against ag-bash's grep
 */
 ...
 */
export async function runGrepTestCase(
  testCase: GrepTestCase,
  options: RunOptions = {},
): Promise<GrepTestResult> {
 ...
 */
export function formatError(result: GrepTestResult): string {
  const lines: string[] = [];
