...
function serializePipeline(node) {
  const prefix = [];
  if (node.timed) {
...
function serializeWordPart(part, inDoubleQuotes) {
  switch (part.type) {
    case "Literal":
...
function serializeParameterOp(param, op) {
  switch (op.type) {
    ...
    default: {
      ...
      throw new Error(
        `Unsupported parameter operation type: ${_exhaustive.type}`,
      );
    }
  }
}
...
function serializeRedirection(node) {
  ...
  if (node.operator === "<<<") {
    return `${fdStr}<<< ${serializeWord(node.target)}`;
  }
  // For &> and &>>, no fd prefix (the & is part of the operator)
  if (node.operator === "&>" || node.operator === "&>>") {
    return `${node.operator} ${serializeWord(node.target)}`;
  }
  return `${fdStr}${node.operator} ${serializeWord(node.target)}`;
}
...
function serializeArithExpr(expr) {
  switch (expr.type) {
    ...
    default: {
      ...
      throw new Error(
        `Unsupported arithmetic expression type: ${_exhaustive.type}`,
      );
    }
  }
}
...
function serializeCondExpr(expr) {
  switch (expr.type) {
    ...
    default: {
      const _exhaustive = expr;
      throw new Error(
        `Unsupported conditional expression type: ${_exhaustive.type}`,
      );
    }
  }
}
