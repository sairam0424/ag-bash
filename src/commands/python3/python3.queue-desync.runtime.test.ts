...
describe("python3 queue runtime desync checks", () => {
  it("isolated python timeout baseline (no shared queue contention)", {
    timeout: 60000,
  }, async () => {
    const bash = new Bash({
      python: true,
  ...
  it("queued timeout request should not leak stdout from an in-flight request", {
    timeout: 60000,
  }, async () => {
  ...
  it("timed-out queued request must not execute later and mutate filesystem", {
    timeout: 60000,
  }, async () => {
    const sharedFs = new InMemoryFs();
