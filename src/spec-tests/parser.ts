...
/**
 * Check if a shell name is bash-compatible (bash, bash-*, or ag-bash)
 */
function isBashCompatible(s: string): boolean {
  return s === "bash" || s.startsWith("bash-") || s === "ag-bash";
}
...
export function getExpectedStdout(testCase: TestCase): string | null {
  // First, look for default stdout (correct behavior) - ag-bash prefers correctness over bug-compatibility
...
export function getExpectedStderr(testCase: TestCase): string | null {
  // First, look for default stderr (correct behavior) - ag-bash prefers correctness over bug-compatibility
  for (const assertion of testCase.assertions) {
    if (
...
export function getExpectedStatus(testCase: TestCase): number | null {
  // First, look for default status (correct behavior - ag-bash prefers correctness)
  for (const assertion of testCase.assertions) {
    if (
...
export function getAcceptableStdouts(testCase: TestCase): string[] {
  ...
  // Add default stdout first (correct behavior - ag-bash prefers correctness)
  for (const assertion of testCase.assertions) {
    if (
      (assertion.type === "stdout" || assertion.type === "stdout-json") &&
      !assertion.shells &&
...
export function getAcceptableStderrs(testCase: TestCase): string[] {
  ...
  // Add default stderr first (correct behavior - ag-bash prefers correctness)
  for (const assertion of testCase.assertions) {
    if (
      (assertion.type === "stderr" || assertion.type === "stderr-json") &&
      !assertion.shells &&
...
export function getAcceptableStatuses(testCase: TestCase): number[] {
  ...
  // Add default status first (correct behavior - ag-bash prefers correctness)
